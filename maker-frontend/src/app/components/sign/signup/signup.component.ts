...
export class SignupComponent implements AfterViewInit {
  ...
  constructor(private store: Store<AppState>, private router: Router, private toast: ToastrService, private trnaslate: TranslateService) {
  ...
  ngAfterViewInit() {
    this.user.pipe(first()).subscribe(userData => {
      if (userData.success === true) {
        combineLatest(this.router.navigateByUrl('/main'), this.trnaslate.get('Sign.up.already'), (route, comment) =>
          this.toast.info(comment)
        ).subscribe();
      } else {
        fromEvent(this.username.nativeElement, 'input')
          .pipe(
            tap(x => (this.nError = true)),
            debounceTime(600)
          )
          .subscribe(data => {
